...
def get_db_info(base_path=None):
    ...
    locations = [
        location
        for location in os.listdir(os.path.join(base_path, db_path))
    ...
    for location in locations:
        ...
        wav_files = [curr_file for curr_file in files if curr_file.endswith("wav")]

        (img_file,) = [curr_file for curr_file in files if curr_file.endswith("jpg")]

        distances = [
            os.path.splitext(wav_file)[0].split(" ")[-1] for wav_file in wav_files
        ]
...
def convolve(source, brir):
    ...
    return np.concatenate(
        [
            np.convolve(source[:, i_channel], brir[:, i_channel])[:, np.newaxis]
